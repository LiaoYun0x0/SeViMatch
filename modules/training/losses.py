...
def alike_distill_loss(kpts, img, class_num):
    ...
    with torch.no_grad():
        ...
        labels = torch.ones((H, W), dtype = torch.long, device = kpts.device) * class_num**2 # -> Default is non-keypoint (bin 64)
        offsets = (((alike_kpts/class_num) - (alike_kpts/class_num).long())*class_num).long()
        offsets =  offsets[:, 0] + class_num*offsets[:, 1]  # Linear IDX
        labels[(alike_kpts[:,1]/class_num).long(), (alike_kpts[:,0]/class_num).long()] = offsets

    kpts = kpts.view(-1,C)
    labels = labels.view(-1)

    mask = labels < class_num**2
...
# 重构损失函数
def reconstruction_loss(x1, x3):
    return F.mse_loss(x1, x3)
